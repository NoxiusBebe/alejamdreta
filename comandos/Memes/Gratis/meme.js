...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "meme (temática)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  try{
    ...
    if(!busqueda){
      meme(function(err2, data) {
        if(err2) return message.channel.send(`Categoria de memes inválida`);
        message.channel.send(new Discord.MessageEmbed().setDescription(data.title).setImage(data.url).setColor(`#88FFC8`).setAuthor(message.author.username, message.author.avatarURL()))
      })
    }
    else{
      meme(`${busqueda}`, function(err2, data) {
        if(err2) return message.channel.send(`Categoria de memes inválida`);
        message.channel.send(new Discord.MessageEmbed().setDescription(data.title).setImage(data.url).setColor(`#88FFC8`).setAuthor(message.author.username, message.author.avatarURL()))
      })
    }
