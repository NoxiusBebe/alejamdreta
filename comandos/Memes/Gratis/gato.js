...
const SomeRandomCat = require('some-random-cat-2').Random;
...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "gato`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  try{
    ...
    SomeRandomCat.getCat().then(url => message.channel.send(new Discord.MessageEmbed().setImage(url.url).setColor(`#88FFC8`).setAuthor(message.author.username, message.author.avatarURL()))).catch(e => { return console.error(e) });
  });
}
