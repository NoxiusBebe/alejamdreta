...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "perro`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  try{
    ...
    got('https://random.dog/woof.json').then(res=>{
        message.channel.send(new Discord.MessageEmbed().setImage(JSON.parse(res.body).url).setColor(`#88FFC8`).setAuthor(message.author.username, message.author.avatarURL()));
    });
  });
}
