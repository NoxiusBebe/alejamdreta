...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "img.desped [elegir: URL tipo png o jpg, OFF]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    let aux = args.join(" ")
    if(!aux) return message.channel.send(new Discord.MessageEmbed().setDescription(`:x: **Y... ¿cuál es la imagen de despedida?**\n\n${estructura}`))

    if(message.guild.me.hasPermission("MANAGE_MESSAGES")) message.delete();

    if(aux==='OFF'){
      db_configuracion.get(`SELECT * FROM servidores WHERE id = ${message.guild.id}`, async (err, filas) => {
        if(err) return console.log(err.message + ` ${message.content} ERROR #1 comando "img_desped" => ${message.content}`)
        let sentencia;
        if(!filas) sentencia = `INSERT INTO servidores(id, img_desped) VALUES('${message.guild.id}', NULL)`;
        else sentencia = `UPDATE servidores SET img_desped = NULL WHERE id = '${message.guild.id}'`;

        db_configuracion.run(sentencia, function(err) {
          if(err) return console.log(err.message + ` ${message.content} ERROR #2 comando "img_desped" => ${message.content}`)
          return message.channel.send(new Discord.MessageEmbed().setDescription(`🗑️ **La imagen de despedida ha sido deshabilitado**`).setColor(`#807C80`))
        })
      })
    }
    else{
      ...
      db_configuracion.get(`SELECT * FROM servidores WHERE id = ${message.guild.id}`, async (err, filas) => {
        if(err) return console.log(err.message + ` ${message.content} ERROR #3 comando "img_desped" => ${message.content}`)
        let sentencia;
        if(!filas) sentencia = `INSERT INTO servidores(id, img_desped) VALUES('${message.guild.id}', '${aux}')`;
        else sentencia = `UPDATE servidores SET img_desped = '${aux}' WHERE id = '${message.guild.id}'`;

        db_configuracion.run(sentencia, function(err) {
          if(err) return console.log(err.message + ` ${message.content} ERROR #4 comando "img_desped" => ${message.content}`)
          return message.channel.send(new Discord.MessageEmbed().setDescription(`:white_check_mark: **Ha sido asignada la imagen de despedida**\n\n__El mensaje es:__\n${aux}`).setColor(`#807C80`))
        })
      })
    }
  })
