...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "voz [frase] | (idioma: es, en, fr, ca)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!decir) return message.channel.send(new Discord.MessageEmbed().setDescription(`:speech_balloon: **Debes decirme qué quieres que diga**\n\n${estructura}`).setColor(`#F7F9F7`))
    ...
    voiceChannel.join().then(connection => {
      ...
      const stream = discordTTS.getVoiceStream(`${parametros[0]}`, {lang: `${parametros[1]}`});
