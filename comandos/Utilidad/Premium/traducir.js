...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "traducir [frase] | (idioma: es, en, fr, ca)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!decir) return message.channel.send(new Discord.MessageEmbed().setDescription(`:speech_balloon: **Debes decirme qué quieres que traduzca**\n\n${estructura}`).setColor(`#F7F9F7`))
    if(!parametros[1]) parametros[1] = "es-ES";
