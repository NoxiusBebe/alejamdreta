...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "guiri [frase] | (idioma: es, en, fr, ca)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!voiceChannel) return message.channel.send(':mute: **Entra a un canal de voz y vuelve a intentarlo.**')
    if(!voiceChannel.permissionsFor(message.guild.me).has("CONNECT")) return message.channel.send(":mute: **No tengo permisos para conectarme a ese canal de voz**")
    if(!decir) return message.channel.send(new Discord.MessageEmbed().setDescription(`:speech_balloon: **Debes decirme qué quieres que diga traducido**\n\n${estructura}`).setColor(`#F7F9F7`))

    let parametros = args.join(" ").split(" | ")
    if(parametros[0].length>200) return message.channel.send(new Discord.MessageEmbed().setDescription(`:pinching_hand: **Límite de 200 carácteres superado.**\n\n${estructura}`).setColor(`#F7F9F7`))
    voiceChannel.join().then(connection => {
      if(!parametros[1]) parametros[1] = "es";
      translate({
        text: parametros[0],
        source: 'auto',
        target: parametros[1]
      }, function(result){
        const stream = discordTTS.getVoiceStream(`${result.translation}`, {lang: `${parametros[1]}`});
