...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "dh.auto.explorar (nivel del 1 al 10)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_discordhunter.get(`SELECT * FROM usuarios WHERE id = '${message.author.id}'`, async (err, filas25) => {
    ...
    db_discordhunter.run(`UPDATE usuarios SET auto_farmear = 1, actividad_auto_farmear = 'Exploracion', coins_auto_farmear = 0, xp_auto_farmear = 0, cantidad_auto_farmeo = 0 WHERE id = '${message.author.id}'`, async function(err) {
    ...
    client.config.sesion_auto_farmeo[message.author.id] = setInterval(async function() {
      db_discordhunter.get(`SELECT * FROM usuarios WHERE id = '${message.author.id}'`, async (err, filas) => {
        ...
        do{
          ...
          if(vida_enemigo<=0){
            let xp_ganados = ((Math.round(Math.random()*(100-50+(nivel/(12-nivel_enemigo))))+(50*nivel_enemigo))*bonificacion)/5;
            let coins_ganados = ((Math.round(Math.random()*(nivel_enemigo*20))+(50*nivel_enemigo))*bonificacion)/5;
