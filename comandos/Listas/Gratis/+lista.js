...
module.exports = async (client, message, args) => {
  ...
  db_listas.all(`SELECT * FROM '${message.guild.id}'`, async (err, filas) => {
    ...
    if(integrantes.some(p => p===`${message.author.id}`)) return message.channel.send(new Discord.MessageEmbed().setDescription(`:thinking: **¿Sabías que no puedo apuntarte 2 veces en la misma lista? Pero tu a lo tuyo...**`).setColor(`#A9FF3D`))
    for(var i=0 ; i<=2000 ; i++){
      if(!filas[numero-1][`user_${i}`]){
        db_listas.run(`UPDATE '${message.guild.id}' SET user_${i} = '${message.author.id}', participantes = ${filas[numero-1].participantes+1} WHERE titulo = '${filas[numero-1].titulo}'`, function(err) {
          if(err) return console.log(err.message + ` ${message.content} ERROR #2 + lista que es ${numero} en ${message.guild.id}`)
          message.channel.send(new Discord.MessageEmbed().setDescription(`:pencil: **Entraste en la lista ${numero}**\n\n**Título:** ${filas[numero-1].titulo}\n**Participantes:** ${filas[numero-1].participantes+1}`).setColor(`#A9FF3D`))
          if(!filas[`user_${i+1}`]){
            db_listas.run(`ALTER TABLE '${message.guild.id}' ADD user_${i+1} TEXT`, function(err) {
              if(err) return console.log(err.message + ` ${message.content} ERROR #3 + lista que es ${numero} en ${message.guild.id}`)
            })
          }
        })
        break;
      }
      else if(filas[numero-1][`user_${i}`]===`${message.author.id}`){
        return message.channel.send(new Discord.MessageEmbed().setDescription(`:thinking: **¿Sabías que no puedo apuntarte 2 veces en la misma lista? Pero tu a lo tuyo...**`).setColor(`#A9FF3D`))
        break;
      }
    }
  })
