...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "evento [@rol] [nombre del evento]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!rol) return message.channel.send(new Discord.MessageEmbed().setDescription(`:face_with_monocle: **Y... ¿el rol?**\n\n${estructura}`).setColor(`#A9FF3D`))
    if(!texto) return message.channel.send(new Discord.MessageEmbed().setDescription(`:face_with_monocle: **¿Y tu evento no tiene nombre o qué?**\n\n${estructura}`).setColor(`#A9FF3D`))

    if(message.guild.me.hasPermission("MANAGE_MESSAGES")) message.delete();
    let mensaje = message.channel.send(new Discord.MessageEmbed().setDescription(`**${texto}**\n\n**👥 Autor:** ${message.author}\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -`).setColor(`#5452BC`).addField(`Rol del evento:`, rol, true).addField(`Participa:`, `🐦`, true).addField(`Cuando acabes:`, `🗑️`, true)).then(m => {
      m.react("🐦");
      setTimeout(function() {
        m.react("🗑️");
      }, 400);
      db_eventos.run(`INSERT INTO eventos(servidor, canal, mensaje, rol, autor) VALUES('${message.guild.id}', '${message.channel.id}', '${m.id}', '${rol.name}', ${message.author.id})`, function(err) {
        if(err) return console.log(err.message + ` ${message.content} ERROR #1 creando un evento`)
      })
    });
  })
}
