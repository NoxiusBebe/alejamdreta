...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "twitter [URL del canal] (@rol)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    let canal_twitter = args.join(" ")

    if(!canal_twitter || !message.content.includes("https://twitter.com/")) return message.channel.send(new Discord.MessageEmbed().setDescription(`:face_with_raised_eyebrow: **Debes poner la URL de la cuenta de Twitter**\n\n${estructura}`).setColor(`#FC5F8B`))

    let role_ping = message.mentions.roles.first();
    if(message.guild.me.hasPermission("MANAGE_MESSAGES")) message.delete();
    let nombre_streamer = []
    for(var i=20 ; i<canal_twitter.length ; i++){
      if(canal_twitter[i]===" ") break;
      nombre_streamer.push(canal_twitter[i])
    }
    db_twitter.run(`CREATE TABLE IF NOT EXISTS '${message.guild.id}' (servidor TEXT, canal TEXT, cuenta TEXT, post TEXT, fecha TEXT, rol TEXT)`, async function(err) {
      if(err) return console.log(err.message + ` ${message.content} ERROR #1 en +tw`)
      db_twitter.get(`SELECT * FROM '${message.guild.id}' WHERE servidor = '${message.guild.id}' AND canal = '${message.channel.id}' AND cuenta = '${nombre_streamer.join("")}'`, async (err, filas) => {
        ...
        db_twitter.run(sentencia, function(err) {
          if(err) return console.log(err.message + ` ${message.content} ERROR #3 en +tw`)
          if(!filas) return message.channel.send(new Discord.MessageEmbed().setDescription(`:inbox_tray: Los post de Twitter de esta cuenta se subiran a este canal`).setColor(`#FC5F8B`))
          else return message.channel.send(new Discord.MessageEmbed().setDescription(`:outbox_tray: Los post de Twitter de esta cuenta que se subían en este canal, ya no se subirán más`).setColor(`#FC5F8B`))
        })
      })
    })
  })
