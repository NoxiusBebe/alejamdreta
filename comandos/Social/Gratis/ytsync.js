...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "ytsync [elegir: 't' {temporal} o 'p' {permanente}] [conectarse a un canal de voz]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  fetch(`https://discord.com/api/v8/channels/${Canalvoz.id}/invites`, {
    ...
    if(invite.error || !invite.code || Number(invite.code)===50013 || invite.code==="50013") return message.channel.send(new Discord.MessageEmbed().setDescription(`:x: **Ha habido un problema con la invitación. Prueba otra vez**`).setColor(`#FC5F8B`))
    return message.channel.send(new Discord.MessageEmbed().setTitle(`:film_frames: ¿Vemos un video juntos? :computer:`).setDescription(`Haced click aquí: <https://discord.gg/${invite.code}>`).setColor(`#FC5F8B`))
  }).catch(e => {
    return message.channel.send(new Discord.MessageEmbed().setDescription(`:x: **Ha habido un problema con la invitación. Prueba otra vez**`).setColor(`#FC5F8B`))
  })
