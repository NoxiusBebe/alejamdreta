...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "monedero`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    db_pizzeria.get(`SELECT * FROM usuarios WHERE id = '${message.author.id}'`, async (err, filas2) => {
      if(err) return console.log(err.message + ` ${message.content} ERROR #1 viendo dinero para pizzas`)
      if(!filas2){
        let embed = new Discord.MessageEmbed()
          .setAuthor(`MONEDERO PARA PIZZAS`, `https://dgusta.cl/wp-content/uploads/2017/09/pizza.gif`)
          .setDescription("**¿Cuánto dinero tienes?:** `50` :coin:\n\n**Cada Pizza cuesta:** `50` :coin:\n\n**Puedes comprar:** `1` pizza/s")
        ...
        let embed = new Discord.MessageEmbed()
          .setAuthor(`MONEDERO PARA PIZZAS`, `https://dgusta.cl/wp-content/uploads/2017/09/pizza.gif`)
          .setDescription('**¿Cuánto dinero tienes?:** `'+`${filas2.dinero}`+'` :coin:\n\n**Cada Pizza cuesta:** `'+'50'+'` :coin:\n\n**Puedes comprar:** `'+`${Math.round(filas2.dinero/50)}`+'` pizza/s')
          .setThumbnail(message.author.avatarURL())
          .setColor("#F49F19")
          .setTimestamp();
        return message.channel.send(embed)
