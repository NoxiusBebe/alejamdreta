...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "bh [conectarse en un canal de voz]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    const dispatcher = await connection.play(`./archivos/Pistas/Bomba de humo/bombahumo.mp3`, {
      volume: 2
    });
  }).catch(err => console.log(`FALLO CON BOMBA DE HUMO`))
    setTimeout(function() {
      Canalvoz.leave();
    }, 7000);
    message.guild.member(message.author.id).voice.setChannel(null);
  })
}
