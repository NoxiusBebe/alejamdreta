...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "sv.ayuda`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    db_tematicos.get(`SELECT * FROM eventos WHERE evento = 'valentin'`, async (err, filas) => {
      ...
      let embed = new Discord.MessageEmbed()
        .setTitle(`:heartpulse: EVENTO DE SAN VALENTÍN :heartpulse: `)
        .setDescription(`Este evento consiste en ser el más rápido en recolectar los ingredientes necesarios para hacer bombones.\n\nUna vez regales los bombones que hayas hecho a otros usuarios, conseguirás puntos.\n\nAquel que obtenga más puntos, ganará el evento.\n`+
        "```\n"+
        client.config.prefijos[message.guild.id]+"sv.canal    ➩ Asigna el canal para jugar\n"+
        client.config.prefijos[message.guild.id]+"sv.piropo   ➩ ¿Quiéres oir un piropo?\n"+
        client.config.prefijos[message.guild.id]+"sv.caja     ➩ Ves tu inventario y estadísticas\n"+
        client.config.prefijos[message.guild.id]+"sv.regalar  ➩ Regala un bombón\n"+
        client.config.prefijos[message.guild.id]+"sv.amorosos ➩ Quiénes regalaron más bombones\n"+
        client.config.prefijos[message.guild.id]+"sv.queridos ➩ Quiénes recibieron más bombones\n"+
        client.config.prefijos[message.guild.id]+"sv.recetas  ➩ Consulta recetas de bombones\n"+
        client.config.prefijos[message.guild.id]+"sv.ayuda    ➩ Ves el menú de ayuda\n"+
        "```")
        .setColor("#FB2727")
        .setThumbnail(`https://cdn.discordapp.com/attachments/523268901719769088/808837044565966858/caja_icono.png`)
