...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "sv.canal`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    db_tematicos.get(`SELECT * FROM eventos WHERE evento = 'valentin'`, async (err, filas) => {
      ...
      db_valentin.get(`SELECT * FROM servidores WHERE id = '${message.guild.id}'`, async (err, filas2) => {
        ...
        let sentencia;
        if(!filas2) sentencia = `INSERT INTO servidores(id,canal) VALUES('${message.guild.id}','${message.channel.id}')`;
        else sentencia = `UPDATE servidores SET canal = '${message.channel.id}' WHERE id = '${message.guild.id}'`;
        db_valentin.run(sentencia, function(err) {
          if(err) return console.log(err.message + ` ${message.content} ERROR #3 ganando puntos`)
          message.channel.send(new Discord.MessageEmbed().setDescription(`:heart_exclamation: Ahora, el amor se siente en este canal...`).setColor("#FB2727"))
        })
      })
    })
  })
