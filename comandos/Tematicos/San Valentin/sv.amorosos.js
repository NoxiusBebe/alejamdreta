...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "sv.amorosos`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    db_tematicos.get(`SELECT * FROM eventos WHERE evento = 'valentin'`, async (err, filas) => {
      ...
      db_valentin.all(`SELECT * FROM '${message.guild.id}'`, async (err, filas2) => {
        if(err) return console.log(err.message + ` ${message.content} ERROR #2 ganando puntos`)
        if(!filas2 || !filas2[0]) return message.channel.send(new Discord.MessageEmbed().setDescription(`:rolling_eyes: **Nadie ha regalado un bombón aún**`).setColor("#FB2727"))
        let frases = [];
        let aux1;
        let aux2;
        let marcador;
        for(var j=0 ; j<10 ; j++){
          for(var i=0 ; i<filas2.length ; i++){
            if(i===0) marcador = i;
            else if(filas2[marcador].regalos<filas2[i].regalos) marcador = i;
            if(i===filas2.length-1){
              if(j===0){
                frases.push(':couplekiss: <@'+filas2[marcador].id+'> | :first_place: '+filas2[marcador].regalos)
                filas2.splice(marcador, 1)
              }
              else if(j===1){
                frases.push(':couplekiss: <@'+filas2[marcador].id+'> | :second_place: '+filas2[marcador].regalos)
                filas2.splice(marcador, 1)
              }
              else if(j===2){
                frases.push(':couplekiss: <@'+filas2[marcador].id+'> | :third_place: '+filas2[marcador].regalos)
                filas2.splice(marcador, 1)
              }
              else{
                frases.push(':couplekiss: <@'+filas2[marcador].id+'> | :gift: '+filas2[marcador].regalos)
                filas2.splice(marcador, 1)
              }
            }
          }
        ...
        let embed = new Discord.MessageEmbed()
          .setTitle(`:sparkling_heart: LOS MÁS AMOROSOS DEL SERVIDOR :sparkling_heart:`)
