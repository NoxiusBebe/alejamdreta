...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "sv.piropo`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    db_tematicos.get(`SELECT * FROM eventos WHERE evento = 'valentin'`, async (err, filas) => {
      ...
      fs.readdir(`./archivos/Pistas/San Valentin`, (err, files) => {
        let iduserafk = message.author.id;
        let Canalvoz = message.guild.member(iduserafk).voice.channel;
        if(!Canalvoz) return message.channel.send(new Discord.MessageEmbed().setDescription(`**${message.author}, NO ESTÁS EN NINGÚN CANAL DE VOZ**`)).then(m => m.delete({ timeout: 10000}))
        else{
          let numaleatorio = (Math.round(Math.random()*files.length-1));
          Canalvoz.join().then(connection => {
            const dispatcher = connection.play(`./archivos/Pistas/San Valentin/${numaleatorio}.mp3`, {
              volume: 2
            });
            dispatcher.on('end', () => {
              Canalvoz.leave();
