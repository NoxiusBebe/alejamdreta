...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "sv.queridos`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    db_tematicos.get(`SELECT * FROM eventos WHERE evento = 'valentin'`, async (err, filas) => {
      ...
      db_valentin.all(`SELECT * FROM '${message.guild.id}'`, async (err, filas2) => {
        ...
        if(!filas2 || !filas2[0]) return message.channel.send(new Discord.MessageEmbed().setDescription(`:rolling_eyes: **Nadie ha recibido un bombón aún**`)).then(m => m.delete({ timeout: 7000}))
        let frases = [];
        let aux1;
        let aux2;
        let marcador;
        for(var j=0 ; j<10 ; j++){
          for(var i=0 ; i<filas2.length ; i++){
            if(i===0) marcador = i;
            else if(filas2[marcador].bombones<filas2[i].bombones) marcador = i;
            if(i===filas2.length-1){
              frases.push(':couple_with_heart: <@'+filas2[marcador].id+'> | :heart: '+filas2[marcador].bombones)
              filas2.splice(marcador, 1)
            }
          }
        }
        let embed = new Discord.MessageEmbed()
          .setTitle(`:gift_heart: LOS MÁS QUERIDOS DEL SERVIDOR :gift_heart:`)
