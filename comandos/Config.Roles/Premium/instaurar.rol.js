...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "instaurar.rol [@rol]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!role) return message.channel.send(new Discord.MessageEmbed().setDescription(`:lying_face: **¿Qué rol quieres instaurar?**\n\n${estructura}`).setColor(`#FF772C`))
    ...
    await message.channel.send(new Discord.MessageEmbed().setDescription(`:pencil: **Instaurando rol... Este proceso tardará: ${T_convertor(miembros.length*1000)} aproximadamente**\n\nTe avisaré cuando termine...`).setFooter(`El tiempo se debe al cumplimiento de las limitaciones de Discord`).setColor(`#FF772C`))

    for(i=0 ; i<miembros.length ; i++){
      let miembro = await message.guild.members.resolve(`${miembros[i]}`);
      if(!miembro.roles.cache.some(r => r == role)){
        await miembro.roles.add(role).catch(console.error);
        contador++;
      }
      if(contador>=10) contador = 0, sleep(10000);
      if(i === miembros.length-1) message.channel.send(`:white_check_mark: **${message.author}, ya he terminado de instaurar el rol ${role}** :thumbsup:`)
    }
