...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "pack.roles`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    let rol;
    try{rol = await message.guild.roles.create({
      data : {
        name: `Administrador`,
        color : "#629BFF",
    ...
    try{rol = await message.guild.roles.create({
      data : {
        name: `Moderador`,
        color : "#FFB145",
        permissions : ["MANAGE_MESSAGES","MANAGE_ROLES","VIEW_AUDIT_LOG","MOVE_MEMBERS","MUTE_MEMBERS","KICK_MEMBERS","BAN_MEMBERS"]
      }
    })}catch{};
    try{rol = await message.guild.roles.create({
      data : {
        name: `Diseñadores`,
        color : "#FF9AFF",
        permissions : ["MANAGE_MESSAGES","MANAGE_ROLES","MANAGE_CHANNELS","VIEW_AUDIT_LOG","MANAGE_WEBHOOKS","MANAGE_EMOJIS"]
      }
    })}catch{};
    try{rol = await message.guild.roles.create({
      data : {
        name: `Iniciante a STAFF`,
        color : "#98FF70",
        permissions : ["MANAGE_MESSAGES","MANAGE_ROLES","VIEW_AUDIT_LOG","MOVE_MEMBERS","MUTE_MEMBERS","MANAGE_EMOJIS"]
      }
    })}catch{};
    try{rol = await message.guild.roles.create({
      data : {
    ...
    return message.channel.send(new Discord.MessageEmbed().setDescription(`:package: **TU PAQUETE DE ROLES PREDETERMINADOR ESTÁ LISTO**\n\nRevisa los roles de tu servidor, deberias tener cosas nuevas :smirk:`).setColor(`#FF772C`))
  })
}
