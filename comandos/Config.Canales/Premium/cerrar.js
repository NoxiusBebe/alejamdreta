...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "cerrar`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    if(err) return console.log(err.message + ` ${message.content} ERROR detectando "premium" en "cerrar" en ${message.guild.id}`)
    ...
    return message.channel.send(new Discord.MessageEmbed().setDescription(`:lock: **CANAL <#${message.channel.id}> BLOQUEADO**`).setColor(`#D6FBAC`)).then(m => m.delete({ timeout: 7000}))
  })
}
