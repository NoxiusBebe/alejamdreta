...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "tempmute [@usuario] [motivo] | [duración con su unidad: s {segundos}, m {minutos}, h {horas}, d {días}]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!reason) return message.channel.send(new Discord.MessageEmbed().setDescription(`:pencil: **Debes incluir el motivo del muteo**\n\n${estructura}`).setColor(`#FF3D5E`))
    if(!temp_limit[1]) return message.channel.send(new Discord.MessageEmbed().setDescription(`:warning: **¿Cuánto tiempo va a estar muteado? ¡No me lo has dicho!**\n\n${estructura}`).setColor(`#FF3D5E`))
    ...
    else return message.channel.send(new Discord.MessageEmbed().setDescription(`:warning: **Debes poner la duración del muteo junto con su unidad**\n\n${estructura}`).setColor(`#FF3D5E`))
    if(isNaN(tiempo_duracion)) return message.channel.send(new Discord.MessageEmbed().setDescription(`:warning: **Debes poner la duración del muteo junto con su unidad**\n\n${estructura}`).setColor(`#FF3D5E`))
    ...
    db_tempmute.run(`INSERT INTO servidores(servidor, usuario, fecha) VALUES('${message.guild.id}', '${user.id}', '${Date.now()+(tiempo_duracion*tiempo_unidad)}')`, function(err) {
      if(err) return console.log(err.message + ` ${message.content} ERROR #0 haciendo tempmute a un usuario`)
    })

    db_canales.get(`SELECT * FROM servidores WHERE id = ${message.guild.id}`, (err, filas) => {
      if(err) return console.log(err.message + ` ${message.content} ERROR #1 haciendo tempmute a un usuario`)
      if(filas && filas.sanciones){
        let embed = new Discord.MessageEmbed()
          .setAuthor(`Usuario desmuteado por ${message.author.tag}`, 'https://cdn.discordapp.com/attachments/823263020246761523/840904595692847114/1f507.png')
          ...
          .addField("Tiempo de muteo: ", T_convertor(tiempo_duracion*tiempo_unidad), false)
          .setTimestamp();
        try{user.send(embed)}catch{}
        try{user.roles.add(role).catch();}catch{}
        try{return client.channels.resolve(filas.sanciones).send(embed);}catch{message.channel.send(new Discord.MessageEmbed().setDescription(`El **MUTEO TEMPORAL** ha sido realizado con éxito. Recuerda asignar un canal para ver todas las sanciones tecleando en el canal que tu elijas **${client.config.prefijos[message.guild.id]}sanciones**`).setColor(`#FF3D5E`)).then(m => m.delete({ timeout: 10000}))}
      }
      else{
        try{user.roles.add(role).catch();}catch{}
        return message.channel.send(new Discord.MessageEmbed().setDescription(`El **MUTEO TEMPORAL** ha sido realizado con éxito. Recuerda asignar un canal para ver todas las sanciones tecleando en el canal que tu elijas **${client.config.prefijos[message.guild.id]}sanciones**`).setColor(`#FF3D5E`)).then(m => m.delete({ timeout: 10000}))
      }
    })
  })
}
