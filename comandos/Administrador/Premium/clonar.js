...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "clonar`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    if(err) return console.log(err.message + ` ${message.content} ERROR detectando "premium" en "clonar" en ${message.guild.id}`)
    ...
    client.channels.cache.get(message.channel.id).clone().then((nuevo_canal) => {
      nuevo_canal.setPosition(client.channels.cache.get(message.channel.id).position)
      client.channels.cache.get(message.channel.id).delete()
      return nuevo_canal.send(new Discord.MessageEmbed().setDescription(`:handshake: **Tu clonación está lista.**`).setColor(`#FF3D5E`))
    })
  })
}
