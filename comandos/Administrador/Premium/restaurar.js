...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "restaurar`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(pausa_restaurar.has(message.guild.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`⛔ **Debes esperar 1 hora para volver a usar este comando** ⛔`).setColor(`#FF3D5E`)).then(m => m.delete({ timeout: 6000}))
    pausa_restaurar.add(message.guild.id);
    setTimeout(() => { pausa_restaurar.delete(message.guild.id); }, 3600000);

    await message.channel.send(new Discord.MessageEmbed().setDescription(`:pencil: **Se está realizando la restauración de apodo. Este proceso tardará: ${T_convertor(miembros.length*1000)} aproximadamente**\n\nTe avisaré cuando termine...`).setFooter(`El tiempo se debe al cumplimiento de las limitaciones de Discord`).setColor(`#FF3D5E`))

    var contador = 0;

    for(var i=0 ; i<miembros.length ; i++){
      let miembro = await message.guild.members.resolve(`${miembros[i]}`);
      if(miembro.nickname) try{await miembro.setNickname(null)}catch{}
      contador++;
      if(contador>=10) contador = 0, sleep(10000);
      if(i === miembros.length-1) message.channel.send(`:white_check_mark: **${message.author}, ya he terminado de restaurar todos los apodos** :thumbsup:`)
    }
  })
}
