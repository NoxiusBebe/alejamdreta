...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "silenciar [conctarse a un canal de voz]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    if(!Canalvoz) return message.channel.send(new Discord.MessageEmbed().setDescription(`:mute: **${message.author}, ¿y dónde quieres que actue con este comando?**\n\n${estructura}`).setColor(`#FF3D5E`))
    ...
    for(i=0;i<arrayserver.length;i++){
      if(arrayserver[i]!=message.author.id){
        ...
        if(!miembro.roles.cache.some(r => r == role)) miembro.roles.add(role).catch(console.error);
      }
    }
    return message.channel.send(new Discord.MessageEmbed().setDescription(`:mute: **¡Silencio!**`).setColor(`#FF3D5E`))
  })
}
