...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "migracion [ID del nuevo canal de voz]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    if(err) return console.log(err.message + ` ${message.content} ERROR detectando "premium" en "migracion" en ${message.guild.id}`)
    ...
    let canal_id = args.join(" ");
    if(!canal_id) return message.channel.send(new Discord.MessageEmbed().setDescription(`:warning: **¿A qué canal quieres migrar?**\n\n${estructura}`).setColor(`#FF3D5E`))
    let Canalvoz = message.guild.member(message.author.id).voice.channel;
    if(!Canalvoz) return message.channel.send(new Discord.MessageEmbed().setDescription(`:mute: **${message.author}, no estás en ningún canal de voz**`).setColor(`#FF3D5E`))
    let miembros = Canalvoz.members.map(member => member.user.id);

    if(pausa_migracion.has(message.guild.id)) return message.channel.send(new Discord.MessageEmbed().setDescription(`⛔ **Debes esperar 1 hora para volver a usar este comando** ⛔`).setColor(`#FF3D5E`)).then(m => m.delete({ timeout: 6000}))
    pausa_migracion.add(message.guild.id);
    setTimeout(() => { pausa_migracion.delete(message.guild.id); }, 3600000);

    await message.channel.send(new Discord.MessageEmbed().setDescription(`:pencil: **Migrando... Este proceso tardará: ${T_convertor(miembros.length*1000)} aproximadamente**\n\nTe avisaré cuando termine...`).setFooter(`El tiempo se debe al cumplimiento de las limitaciones de Discord`).setColor(`#FF3D5E`))

    var contador = 0;

    for(var i=0 ; i<miembros.length ; i++){
      let miembro = await message.guild.members.resolve(`${miembros[i]}`);
      if(message.guild.member(miembro).voice.channel) try{message.guild.member(miembro).voice.setChannel(canal_id);}catch{}
      contador++;
      if(contador>=10) contador = 0, sleep(10000);
      if(i === miembros.length-1) message.channel.send(`:white_check_mark: **${message.author}, ya he terminado de migrar a los usuarios** :thumbsup:`)
    }
  })
}
