...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "desestimar [@usuario] [motivo]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  if(!reason) return message.channel.send(new Discord.MessageEmbed().setDescription(`:warning: **¡Debes de dar un motivo para la desestimacion!**\n\n${estructura}`).setColor(`#FF3D5E`))
  ...
  db_strikes.run(`CREATE TABLE IF NOT EXISTS '${message.guild.id}' (usuario TEXT, strikes INTEGER)`, async function(err) {
    ...
    db_canales.get(`SELECT * FROM servidores WHERE id = ${message.guild.id}`, async (err, filas) => {
      ...
      db_strikes.get(`SELECT * FROM '${message.guild.id}' WHERE usuario = ${user.id}`, async (err, filas2) => {
        ...
        if(!filas2) sentencia = `INSERT INTO '${message.guild.id}'(usuario, strikes) VALUES(${user.id}, 0)`, num_warns = 0;
        ...
        db_strikes.run(sentencia, async function(err) {
          ...
          if(filas && filas.sanciones){
            let embed = new Discord.MessageEmbed()
              .setAuthor(`Reporte de usuario desestimado por ${message.author.tag}`, 'https://cdn.discordapp.com/attachments/823263020246761523/931373390118391828/appeals-icon.png')
              .setDescription(`**Motivo:** ${reason}`)
              .setColor("#1FAE33")
              .setThumbnail(user.user.displayAvatarURL())
              .addField("Servidor: ", message.guild.name, true)
              .addField("Desestimación a: ", `${user}`, true)
              .addField("Desestimado en: ", message.channel, true)
              .addField("Número de reportes: ", num_warns, true)
              .setTimestamp();
            try{await user.send(embed)}catch{};
            try{await client.channels.resolve(filas.sanciones).send(embed);}catch{message.channel.send(new Discord.MessageEmbed().setDescription(`La **DESESTIMACION** ha sido adjudicado con éxito. Recuerda asignar un canal para ver todas las sanciones tecleando en el canal que tu elijas **${client.config.prefijos[message.guild.id]}sanciones**`).setColor(`#FF3D5E`)).then(m => m.delete({ timeout: 10000}))}
          }
          else message.channel.send(new Discord.MessageEmbed().setDescription(`La **DESESTIMACION** ha sido adjudicado con éxito. Recuerda asignar un canal para ver todas las sanciones tecleando en el canal que tu elijas **${client.config.prefijos[message.guild.id]}sanciones**`).setColor(`#FF3D5E`)).then(m => m.delete({ timeout: 10000}))
        })
      })
    })
  })
