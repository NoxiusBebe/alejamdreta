...
module.exports = async (client, message, args, canal_dj) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "archivo [adjunta un archivo .mp3]`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    let archivo = message.attachments.map(c => c.attachment)
    let nombre = message.attachments.map(c => c.name)

    if(!archivo || archivo.length<1) return message.channel.send(new Discord.MessageEmbed().setDescription(`:x: **${message.author} debes adjuntar el archivo que quieras reproducir**\n\n${estructura}`).setColor(`#39F0BE`))

    Canalvoz.join().then(connection => {
      const dispatcher = connection.play(`${archivo[0]}`);
      message.delete();
      message.channel.send(new Discord.MessageEmbed().setDescription(":headphones: **Reproduciendo:** `"+nombre[0]+"`\n\n:satellite: **Descarga este archivo [haciendo click aqui]("+archivo[0]+")**").setColor(`#39F0BE`).setFooter(message.author.tag, message.author.avatarURL()).setTimestamp())
    }).catch(console.error);
