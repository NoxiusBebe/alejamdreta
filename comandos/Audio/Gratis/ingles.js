...
let direccion = "./archivos/Pistas/Ingles";
...
module.exports = async (client, message, args, canal_dj) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "ingles (número del audio: del 1 al 43)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  fs.readdir(`${direccion}`, (err, files) => {
    let numaleatorio = (Math.round(Math.random()*files.length-1));
    let seleccion = parseInt(args.join(" "))
    if(seleccion && !isNaN(seleccion) && seleccion>=1 && seleccion<=files.length) numaleatorio = seleccion-1;

    if(canal_dj){
      canal_dj.join().then(async connection => {
        const dispatcher = await connection.play(`${direccion}/Grabacion${numaleatorio}.mp3`, {
          volume: 2
        });
        message.channel.send(new Discord.MessageEmbed().setDescription(`:headphones: Listen...`).setColor(`#39F0BE`))
        dispatcher.on("finish",() => canal_dj.leave());
      }).catch(err => console.log(`ERROR #1 comando "ingles" => ${message.content}`))
    }
    else{
