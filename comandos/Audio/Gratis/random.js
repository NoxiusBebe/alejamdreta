...
let direccion = "./archivos/Pistas/Random";
...
module.exports = async (client, message, args, canal_dj) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "random (número del audio: del 1 al 32)`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  fs.readdir(`${direccion}`, (err, files) => {
    ...
    if(canal_dj){
      canal_dj.join().then(async connection => {
        const dispatcher = await connection.play(`${direccion}/${numaleatorio}.mp3`, {
          volume: 2
        });
        message.channel.send(new Discord.MessageEmbed().setDescription(`:headphones: ¿Qué será?...`).setColor(`#39F0BE`))
        dispatcher.on("finish",() => canal_dj.leave());
      }).catch(err => console.log(`ERROR #1 comando "random" => ${message.content}`))
    }
    else{
