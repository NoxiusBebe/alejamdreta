...
module.exports = async (client, message, args, canal_dj) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "fox`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  let pista;
  if(numaleatorio===4) pista = 'fox2';
  else pista = 'fox';

  if(canal_dj){
    canal_dj.join().then(async connection => {
      const dispatcher = await connection.play(`${direccion}/${pista}.mp3`, {
        volume: 2
      });
      message.channel.send(new Discord.MessageEmbed().setDescription(`:headphones: Fox...`).setColor(`#39F0BE`))
      dispatcher.on("finish",() => canal_dj.leave());
    }).catch(err => console.log(`ERROR #1 comando "fox" => ${message.content}`))
  }
  else{
