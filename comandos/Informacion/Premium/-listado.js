...
module.exports = async (client, message, args) => {
  ...
  let estructura = "__Forma de usar el comando:__ `" + client.config.prefijos[message.guild.id] + "-listado`\n" + "```js\n" + "[] -> Obligatorio\n" + "() -> Opcional\n" + "{} -> Aclaración\n" + "```";
  ...
  db_premium.get(`SELECT * FROM premium WHERE servidor = '${message.guild.id}'`, async (err, filas) => {
    ...
    /*if(!filas || (filas.premium===null && filas.informacion===null)){
      let embed_no = new Discord.MessageEmbed()
        ...
        .attachFiles([new MessageAttachment(`${imagen_premium}/premium-informacion.png`, `premium.png`)])
    ...
    db_servidores.run(`DELETE FROM servidores WHERE id = '${message.guild.id}'`, function() {
      if(err) return console.log(err.message + ` ${message.content} ERROR #3 en el comando "-listado"`)
      return message.channel.send(new Discord.MessageEmbed().setDescription(`:outbox_tray: **El servidor ha sido eliminado del listado del bot**`).setColor(`#ACC5FB`))
    })
  })
}
